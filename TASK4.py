...
W = tf.Variable(np.random.normal(0, 0.05, (hiddenUnits + hiddenUnits + 8, hiddenUnits)), dtype=tf.float32)
...
# create the sequence for 10 words each time
for number in range(maxSeqLen-10):
    state = sequenceOfLetters[number]
    for i in range(9):
        state = tf.concat([state, sequenceOfLetters[number+i]], 1)
...
state_record = {}
count = 0
filter_matrix = tf.Variable(np.random.normal(0, 0.05, (2560, 8)), dtype=tf.float32)
for timeTick in new_sequence:
    #
    # concatenate the state with the input, then compute the next state
    if count < 10:
        inputPlusState = tf.concat([tf.matmul(timeTick, filter_matrix), currentState, np.zeros((batchSize, hiddenUnits))], 1)
    else:
        inputPlusState = tf.concat([tf.matmul(timeTick, filter_matrix), currentState, state_record[count % 10]], 1)
